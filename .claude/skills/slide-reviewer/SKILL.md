...
1. `lectures/{topic}/slides/` 디렉토리에서 Slidev dev 서버를 시작한다 (사용하지 않는 포트 자동 선택)
2. Playwright로 모든 슬라이드를 `980x552` 뷰포트에서 캡처한다
   - `?clicks=99` 파라미터로 모든 v-click을 펼친 상태로 캡처
   - 저장 경로: `lectures/{topic}/slides/screenshots-review/slide-{NNN}.png`
3. 서버를 종료한다

캡처 스크립트 `scripts/capture-slide.mjs`를 사용한다:
...
스크립트가 자동으로:
- Slidev dev 서버를 포트 3098에서 시작
- `980x552` 뷰포트로 Playwright 캡처 (`?clicks=99`로 v-click 펼침)
