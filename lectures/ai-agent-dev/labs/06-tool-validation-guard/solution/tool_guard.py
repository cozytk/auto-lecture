...
def schema_validation(state: ValidationState) -> dict:
    ...
    for field in schema["required"]:
        if field not in state["tool_args"]:
            errors.append(f"missing field: {field}")

    for field, (minimum, maximum) in schema["ranges"].items():
        value = state["tool_args"].get(field)
        if value is not None and not (minimum <= value <= maximum):
            errors.append(f"out of range: {field}={value}")
...
def policy_validation(state: ValidationState) -> dict:
    ...
    if state["call_count"] >= state["max_calls"]:
        errors.append("call limit exceeded")
    if policy["requires_approval"] and not state["approved_by_human"]:
...
def result_validation(state: ValidationState) -> dict:
    ...
    if state["requested_tool"] == "search_docs" and not result.get("items"):
        errors.append("empty search result")
    if state["requested_tool"] == "delete_ticket" and not result.get("deleted"):
        errors.append("missing deleted ticket id")
...
def run_demo() -> None:
    ...
    scenarios = [
        ...
        {
            "requested_tool": "delete_ticket",
            "tool_args": {"ticket_id": "INC-9"},
            "schema_ok": False,
            "policy_ok": False,
            "context_ok": False,
            "result_ok": False,
