...
> 실습 상세는 `labs/day1/s1-agent-problem-definition/README.md` 참고

### 시연 (30분)

강사가 PydanticAI 기반 Agent 3가지 패턴을 Jupyter Notebook으로 실행하며 시연한다.

- `src/01_pydantic_ai_basics.ipynb` — PydanticAI 기본
- `src/02_automation_agent.ipynb` — 자동화형 Agent (GitHub 이슈 일일 보고)
- `src/03_analysis_agent.ipynb` — 분석형 Agent (코드 리뷰 피드백)
- `src/04_planner_agent.ipynb` — Planner형 Agent (기술 조사 보고서)

### 과제 (45분)

개인 업무 기반 Agent 후보 2-3개를 Pain → Task → Skill → Tool 형식으로 노션 데이터베이스에 설계한다.

### 공유 (15분)

2-3명이 Agent 후보를 발표하고 피드백을 주고받는다.
