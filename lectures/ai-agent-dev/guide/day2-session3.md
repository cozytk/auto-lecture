...
<callout icon="📖" color="blue_bg">
	**학습 목표**
	1. Pydantic args_schema로 Tool 스키마를 강제할 수 있다
	2. 사전/사후 검증 레이어를 설계하고 구현할 수 있다
	3. 미들웨어 패턴으로 Tool 호출을 인터셉트할 수 있다
	4. 무한 루프 방지 전략을 구현할 수 있다
</callout>
...
<callout icon="💡" color="gray_bg">
	이 세션의 실습은 `labs/day2/02_tool_middleware/` 노트북을 사용한다.
	Pydantic 스키마와 미들웨어로 Tool 호출을 제어한다.
</callout>

### 🔍 I DO: Tool 스키마와 Structured Output {toggle="true"}
	**시간**: 25분
	**노트북**: `02_tool_middleware/01_tools.ipynb`

	강사가 라이브로 Tool 스키마와 구조화된 출력을 구현한다.
	- `@tool` + Pydantic `args_schema` 정의
	- `with_structured_output()`으로 JSON 응답 강제
	- ToolRuntime으로 동적 도구 접근
	<callout icon="💡" color="gray_bg">
		**관찰 포인트**: Field description이 LLM의 Tool 선택 정확도에 미치는 영향을 확인한다.
	</callout>

### 🤝 WE DO: 미들웨어로 Tool 통제 {toggle="true"}
	**시간**: 25분
	**노트북**: `02_tool_middleware/02_middleware.ipynb`

	미들웨어 패턴으로 Tool 호출을 인터셉트한다.
	- [ ] `@wrap_tool_call`로 실행 시간 측정
	- [ ] `ToolCallLimitMiddleware`로 최대 호출 횟수 제한
	- [ ] `ModelCallLimitMiddleware`로 무한 루프 방지
	- [ ] 커스텀 미들웨어 작성 (감사 로깅)

### 🚀 YOU DO: 검증 파이프라인 설계 {toggle="true"}
	**시간**: 10분

	<callout icon="📋" color="yellow_bg">
		**요구사항**
		1. 사전 검증(권한, 비용)과 사후 검증(결과 품질)을 포함하는 Tool 검증 파이프라인을 설계한다
		2. Fallback 체인을 구현한다 (Primary → Secondary → LLM Fallback)
		3. 루프 방지: 동일 Tool에 3회 이상 연속 호출 시 자동 중단
	</callout>

<details>
<summary>💡 힌트</summary>
	미들웨어 실행 순서가 중요하다. 비용 검증 → 권한 검증 → 실행 → 결과 검증 순으로 파이프라인을 구성한다.
</details>
