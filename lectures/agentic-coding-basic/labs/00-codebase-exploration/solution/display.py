...
def render_goal(today_minutes, goal_minutes):
    """일일 학습 목표 진행 상황 렌더링."""
    bar = _render_progress_bar(today_minutes, goal_minutes)
    done = today_minutes >= goal_minutes
    status = _c(_GREEN, "달성!") if done else _c(_YELLOW, "진행 중")
    lines = [
        _c(_BOLD, "🎯 일일 학습 목표"),
        "",
        f"  목표: {format_duration(goal_minutes)}",
        f"  오늘: {format_duration(today_minutes)}",
        f"  진행: {bar}  {status}",
    ]
    return "\n".join(lines)
