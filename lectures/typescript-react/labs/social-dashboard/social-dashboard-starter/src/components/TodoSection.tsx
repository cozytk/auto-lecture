...
interface TodoSectionProps {
  userId: number;
  onStatsUpdate?: StatsUpdateFn; // 도전 11: 콜백으로 App의 stats 갱신
}
...
export default function TodoSection({ userId, onStatsUpdate }: TodoSectionProps) {
  ...
  //
  // TODO (도전 11): 토글 후 onStatsUpdate 콜백을 호출하여 App의 stats.todoDone을 갱신하세요
  //   - 완료로 바뀌면: onStatsUpdate?.("todoDone", prev => prev + 1)
  //   - 미완료로 바뀌면: onStatsUpdate?.("todoDone", prev => prev - 1)
  const toggleTodo = (id: number): void => {
    // 여기에 구현하세요
  };

  // 필터링 로직 (완성됨)
  const todos = JSON.parse("[]") as Todo[]; // TODO: 위에서 만든 state를 사용하므로 이 줄을 삭제하세요
